...
class MainUI(QMainWindow):
    def __init__(self):
        super().__init__()  # ⚠️ 这行绝对不能少！否则上面错误
    ...
    def _fortissimo_tab(self):
        ...
        l.addWidget(title)
        l.addWidget(QLabel("前台模式命中率高达 98% 以上 · 游戏窗口自动置顶",
                            alignment=Qt.AlignCenter, styleSheet="color:#00ff88; font-size:13px;"))
        l.addWidget(QLabel("后台模式命中率可达 80% 以上 · 无额外限制",
                            alignment=Qt.AlignCenter, styleSheet="color:#ffaa00; font-size:13px;"))
        l.addSpacing(20)
        ...
        b1.setStyleSheet("QPushButton{background:#0a0a2a; border:2px solid #00ffff; border-radius:10px; color:#00ffff;} "
                         "QPushButton:hover{background:#00ffff; color:#050510;}")
        b2 = QPushButton("后台模式")
        b2.setMinimumSize(150, 60)
        b2.setFont(QFont("Microsoft YaHei", 13))
        b2.setStyleSheet("QPushButton{background:#0a0a2a; border:2px solid #ffaa00; border-radius:10px; color:#ffaa00;} "
                         "QPushButton:hover{background:#ffaa00; color:#050510;}")
        btn_layout.addWidget(b1)
        btn_layout.addWidget(b2)
        l.addLayout(btn_layout)
        l.addWidget(QLabel("※ 前台模式会自动保持游戏窗口置顶，确保按键精准。",
                            alignment=Qt.AlignCenter, styleSheet="color:#888888; font-size:11px; margin-top:15px;"))
